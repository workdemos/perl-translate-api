...
our $API_TRANS ="http://admin.max.com/api/remote_act?a=translate_item_app";
...
sub prepare_items{
    ...
    my $rows = $dbh->selectall_hashref("SELECT id,app_id,name_jp,marketing_txt,revision from items_for_trans WHERE status='y' order by translated limit 3 ","app_id");
    return $rows;
}
